...
export default function RequestDetail() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!req) { navigate('/game/psx/p2is/requests'); return }
      setRequest(req)
  ...
  async function deleteRequest() {
    if (!request || (!isAuthor && !isOwner)) return
    if (!confirm('确定删除此请求？此操作不可撤销。')) return
    const { error } = await supabase.from('merge_requests').delete().eq('id', request.id)
    if (error) { alert('删除失败：' + error.message); return }
    navigate('/game/psx/p2is/requests')
  }

  if (loading) return (
    <div className="p2is-page"><TopNav /><div className="browse-wrap"><p className="muted">加载中…</p></div></div>
  )
    ...
    <div className="p2is-page">
      ...
      <div className="browse-wrap">
        ...
        <div className="request-detail-header">
          <button className="btn-ghost" style={{ marginBottom: 16 }} onClick={() => navigate('/game/psx/p2is/requests')}>← 返回</button>
